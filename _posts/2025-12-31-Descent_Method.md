...
which implies that $\lim_{T\rightarrow \infty}\|\|\nabla f(x_{T})\|\| = 0$ (<span style="color:red">Note that the sequence of partial sums ${S_{j} = \sum_{k=0}^{j}}\|\|\nabla f(x_{k})\|\|^{2}$ is non-decreasing and upper-bounded. Therefore, the series converges. A necessary condition for series convergence implies that the terms of the sum must approach zero, i.e., $\|\|\nabla f(x_{k})\|\| \rightarrow 0$ as $k \rightarrow \infty$</span>). Moreover, we have
...
Before diving into the details of the convergence analysis, we first talk about two simple facts.
...
We are in favor of strongly convex functions because of its nice properties. First, the norm of the gradient can provide information about how far away we are from optimality. Given the property of the stronlgy convex function $f$,
...
Obviously, the iteration of strongly convex case depends logarithmically on $\epsilon$, while the other two cases depend on $1/\epsilon$ and $1/\epsilon^{2}$ respectively. To illustrate the practical implication, consider a typical stopping criterion $\epsilon = 10^{-6}$. In the worst (general) case, the algorithm may require up to $10^{12}$ iterations, while the strongly convex case needs merely $\ln(10^{6})$ iterations.

### Descent Method: Convergence

In the previous section, all of the derivations depend on the knowledge of Lipschitz constant $L$. In this section, we prove convergence results for more general descent methods.

We assume that the following inequality holds,

$$
f(x_{k+1}) \le f(x_{k}) - C \|\nabla f(x_{k})\|^{2}, \quad \mathrm{for~some}~C > 0.
$$

Then we have the following elementary theorem.

##### Theorem 2

Suppose that $f$ is bounded below, with Lipschitz continuous gradient. Then all accumulation points $\bar{x}$ of the sequence $\{x_{k}\}$ generated by a scheme that satisfies the above assumption are stationary; that is, $\nabla f(\bar{x}) = 0$. If, in addition, $f$ is convex, each such $\bar{x}$ is a solution of the problem $\min_{x\in \mathbb{R}^{n}} f(x)$.
...
$$
\|\nabla f(x_{k})\|^{2} \le \frac{f(x_{k}) - f(x_{k+1})}{C},
$$

$\{f(x_{k})\}$ is therefore a decreasing sequence. Additionally, $f$ is bounded below. We can conclude $\{f(x_{k})\}$ is convergent. It follows, $\lim_{k\rightarrow \infty}f(x_{k}) - f(x_{k+1})=0$. If $\bar{x}$ is an accumulation point, then there's a subsequence $\mathcal{S}$ such that $\lim_{k\in \mathcal{S},k\rightarrow\infty}x_{k} = \bar{x}$. By the continuity of $\nabla f$, we have $\nabla f(\bar{x}) = \lim_{k\in\mathcal{S},k\rightarrow\infty}\nabla f(x_{k}) = 0$ <span style="color:blue">(Sandwich theorem: $0 \le \|\|\nabla f(x_{k})\|\|^{2} \le \frac{f(x_{k}) - f(x_{k+1})}{C}$)</span>. If $f$ is convex, each $\bar{x}$ satisfies the first-order sufficient condition is a solution.
